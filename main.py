...
class AplicacionValidadorContrasena:
    ...
    def __init__(self, interfaz):
        ...
        self.etiqueta_titulo = ctk.CTkLabel(self.marco_izquierdo, text="Validador de Contraseñas",
        ...
        self.entrada_contrasena = ctk.CTkEntry(self.marco_izquierdo, placeholder_text="Contraseña", width=350,
                                               height=40, border_width=2, corner_radius=10, font=ctk.CTkFont(size=15))
        self.entrada_contrasena.grid(row=1, column=0, pady=(0, 15), padx=20, sticky="ew", columnspan=2)
        ...
        self.frame_botones.grid_columnconfigure(0, weight=4)  # Botón validar más ancho
        self.frame_botones.grid_columnconfigure(1, weight=1)  # Botón limpiar más estrecho

        # Botón validar
        self.boton_validar = ctk.CTkButton(self.frame_botones, text="Validar Contraseña",
                                           command=self.validar_contrasena, height=40, fg_color="#3498db",
                                           hover_color="#2980b9", font=ctk.CTkFont(size=16), corner_radius=10)
        self.boton_validar.grid(row=0, column=0, padx=(0, 5), sticky="ew")

        # Botón limpiar (pequeño con icono)
        ...
        self.etiqueta_expresion_titulo = ctk.CTkLabel(self.marco_expresion, text="Expresión Regular",
        ...
        self.etiqueta_expresion = ctk.CTkLabel(self.marco_expresion, text="", font=ctk.CTkFont(size=12), wraplength=380,
        ...
        self.area_texto_grande = ctk.CTkTextbox(self.marco_izquierdo, width=350, height=200, font=ctk.CTkFont(size=14),
                                                corner_radius=10, border_width=2)
        self.area_texto_grande.grid(row=6, column=0, pady=(5, 20), padx=20, sticky="ew", columnspan=2)
        ...
        self.etiqueta_titulo_requisitos = ctk.CTkLabel(self.marco_derecho, text="Requisitos de la Contraseña",
                                                       font=ctk.CTkFont(size=22, weight="bold"), anchor="center")
        ...
        for i, texto in enumerate(requisitos_texto):
            ...
            etiqueta = ctk.CTkLabel(marco_req, text=f"{requisitos_iconos[i]} {texto}", font=ctk.CTkFont(size=16),
                                    pady=5)
    ...
    def verificar_requisitos(self, contrasena):
        ...
        for i, etiqueta in enumerate(self.requisitos_labels):
            etiqueta.configure(text=f"{requisitos_iconos[i]} {textos_base[i]} {'✓' if cumple[i] else '✗'}",
                               text_color="green" if cumple[i] else "red")
        return all(cumple)
    ...
    def limpiar_campos(self):
        # Limpia la entrada de contraseña
        self.entrada_contrasena.delete(0, 'end')

        # Limpia el área de texto de análisis
        self.area_texto_grande.delete(0.0, "end")

        # Limpia la etiqueta de resultado
        self.etiqueta_resultado.configure(text="", text_color="black")

        # Limpia la etiqueta de expresión regular
        self.etiqueta_expresion.configure(text="", text_color="black")

        # Resetea los requisitos a su estado original
        self.resetear_requisitos()
